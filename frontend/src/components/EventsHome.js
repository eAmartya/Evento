...
function EventsHome() {
  ...
  useEffect(() => {
    axios
      .get("https://wax-nostalgic-macaroni.glitch.me/api/event/getEvent")
      .then((response) => {
      ...
      <div className="flex h-full flex-col lg:flex-row ">
        ...
        <div className="lg:w-screen text-white mt-4 ml-4 mr-2">
          <div className="popular-events">
            <p className="flex text-4xl bg-white text-black font-medium p-5 lg:w-full rounded-xl ">
              Popular Events
            </p>
              ...
              {popularEvents.length !== 0 ? (
                popularEvents.map((ele) => {
                  return (
                    <EventCard
                      key={ele._id}
                      dataToFetch={ele.eventName}
                      eventName={
                        ele.eventName.charAt(0).toUpperCase() +
                        ele.eventName.slice(1)
                      }
                      date={ele.startDate}
                      eventCategory={ele.eventType}
                      eventImage={ele.eventImage}
                    />
                  );
                })
              ) : (
          ...
          <div className="sports-events mt-4">
            <p className=" flex text-4xl bg-white text-black font-medium p-5 lg:w-full rounded-xl ">
              <svg
                ...
                height="1em"
                viewBox="0 0 512 512"
                className="pr-2 text-indigo-700"
              >
